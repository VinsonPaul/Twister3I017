...
public class RemoveMessage {
	...
	public static JSONObject removeMessage(String username, String content, String date, String type, String key) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, JSONException {
		if (toolUser.checCo(username,key)) {
			if(toolMessage.messageExist(username,content,date)) {
				if(Message_BD.removeMessage(username,content,date,type)) {
