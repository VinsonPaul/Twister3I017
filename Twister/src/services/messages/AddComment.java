package services.messages;

import java.sql.SQLException;
import java.util.Date;

import org.json.JSONException;
import org.json.JSONObject;

import bdd.Message_BD;
import bdd.User_BD;
import tools.ErrorClass;
import tools.ErrorCode;
import tools.toolUser;

public class AddComment {

	public static JSONObject addComment (String username, String content, String username_to, String content_to, String date_to, String key) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException, JSONException {
		if (toolUser.checCo(username,key)) {
			Date d = Message_BD.addComment(username, content, username_to, content_to, date_to);
			if(d != null) {
				JSONObject o = new JSONObject();
				o.append("username", username);
				o.append("key", key);
				o.append("content", content);
				o.append("date", d);
				o.append("username_to", username_to);
				o.append("content_to", content_to);
				o.append("date_to", date_to);
				o.append("id", User_BD.getID(username));
				o.append("id_to", User_BD.getID(username_to));
				return ErrorClass.serviceAccepted(o);
			}
			return ErrorCode.DATABASE_ADD_COMMENT.toJSON();
		}
		return ErrorCode.USER_NOT_LOGED.toJSON();
	}
	
}
